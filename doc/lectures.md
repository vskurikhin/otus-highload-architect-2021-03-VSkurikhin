...
новременных соединений: Simultaneous connections Concurrency
...
- Перцентиль (процент) — значение антропометрического признака для сотой доли совокупности измеренных людей.
  Если кривую распределения всей совокупности измеренных людей разделить на 100 равных частей, то получим 99 площадей,
  в каждой из которых будет свое значение признака и частота ее встречаемости.

Виды масштабирования
- Вертикальное - увеличение мощности сервера.
- Горизонтальное - использование бОльшего колличества серверов.

### [Индексы часть 1](https://otus.ru/lessons/highloadarchitect/?int_source=courses_catalog&int_term=programming)

Индекс - 
служебная структура данных
некий способ отображения ключа в данные ускоряют поиск, с?
...
будем использовать внешний носитель (привет, медленный HDD) logN обращений к медленной памяти
...
Поиск в B-tree
алгоритм аналогичен бинарному, но дальнейший выбор не из 2х, а из нескольких поиск за O(t logt(n))
НО обращений к диску O(logt(n))

Добавление в B-tree
нельзя добавить ключ в уже заполненный узел разбиение на 2 по t-1 элементу
может привести к увеличение высоты добавление за O(t logt(n))
НО обращений к диску O(logt(n))

Какои мне взять?
больше t => меньше высота дерева зависит от размера блока на диске зависит от объема оперативной памяти обычно от 50 до 2000
t=1001 и 1млрд записей => 3 операции для любого ключа
